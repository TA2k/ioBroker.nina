        ...
        Object.keys(this.status[areaCode].buckets).forEach((bucket) => {
          this.status[areaCode].buckets[bucket].forEach((ref) => {
            if (this.config.ignoreDwd && bucket === 'bbk.dwd') {
              ...
              this.status[areaCode].activeWarn = parseInt(this.status[areaCode].activeWarn) - 1;
              return;
            }
                  ...
                  try {
                    ...
                    if (this.config.ignoreFilterText) {
                      ...
                      if (found) {
                        this.status[areaCode].numberOfWarn = parseInt(this.status[areaCode].numberOfWarn) - 1;
                        this.status[areaCode].activeWarn = parseInt(this.status[areaCode].activeWarn) - 1;
                        this.setState(areaCode + '.ignoreIdentifierList', {
                          val: JSON.stringify(this.ignoreIdentifierList),
                          ack: true,
                        });
                        resolve();
                        return;
  ...
  setGefahren() {
    return new Promise((resolve) => {
      ...
      Object.keys(this.status).forEach((areaCode) => {
        ...
        this.setState(areaCode + '.identifierList', {
          val: JSON.stringify(this.status[areaCode].identifierList),
          ack: true,
        });
      });
